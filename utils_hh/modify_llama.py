...
from .modeling_llama import LlamaRotaryEmbedding, LlamaAttention, apply_rotary_pos_emb, LlamaLinearScalingRotaryEmbedding, LlamaDynamicNTKScalingRotaryEmbedding
...
class LlamaAttention_heavy_hitter(nn.Module):
    ...
    def __init__(self, layerid, config: LlamaConfig):
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        value_l1norm=torch.norm(value_states, p=1, dim=-1)[0]
        ...
        if past_key_value is not None:
            ...
            self.value_norm=torch.cat([self.value_norm,value_l1norm], dim=1)
        else:
            self.value_norm=value_l1norm
        ...
        if not self.previous_scores == None:
            ...
            # if self.applyval:
            if not self.h2o:
                current_scores_sum = attn_weights[:,:, -400: , :].sum(0).sum(1)
                current_scores_sum =current_scores_sum / 400
                self.heavy_budget = int(self.heavy_budget_ratio * current_scores_sum.shape[-1])*2-10
                #streamLLM
                # self.heavy_budget=self.sink_len
                # self.recent_budget = int(self.recent_budget_ratio * current_scores_sum.shape[-1])
                self.recent_budget = 10
                if self.applyval:
                    current_scores_sum=current_scores_sum*self.value_norm
            else:
...
def convert_kvcache_llama_heavy_recent(model, config):
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, LlamaAttention):
            print(f"convert:{name}layerid{model._modules[name].layerid}")
            model._modules[name] = LlamaAttention_heavy_hitter(model._modules[name].layerid, config)
